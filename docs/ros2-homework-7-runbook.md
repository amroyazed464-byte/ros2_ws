...
`LIBGL_ALWAYS_SOFTWARE=1` 强制 Mesa 使用 CPU 软件渲染，用于规避 VMware 中
Gazebo 白屏、黑屏或 OpenGL 不兼容。该变量只对当前终端及其子进程生效，不会永久
修改系统配置；软件渲染的 CPU 占用通常高于硬件加速。如果 Gazebo 能正常使用硬件
渲染，可省略这一行。如果 RViz 也出现白屏，在启动 Navigation2 的终端中同样先执行：
...
电量高于 20% 时按约每秒 0.5% 消耗。电量达到或低于 20% 后，这部分剩余电量被视为
返航专用保留电量：调度器取消当前搬运目标并导航到充电站，返航期间暂停继续扣电，
从而不受地图位置、规划耗时或 Gazebo 实时率影响。充电成功后电量恢复到 100%，
再恢复此前被中断的搬运目标。
...
- Gazebo 窗口白屏、黑屏或场景不显示时，停止 Gazebo，在同一终端执行
  `export LIBGL_ALWAYS_SOFTWARE=1` 后重新启动 World。该设置强制使用 CPU
  软件渲染。
...
4. 电量达到或低于 `20.0` 时，确认当前导航任务被取消，且日志记录返航充电；返航
   期间电量保持在触发恢复时的数值，不再继续下降。
